...
# Đường dẫn lưu ảnh
save_directory = "C:/Users/magic/Desktop/Nghiên_cứu_khoa_học/Fall_Detection_Using_Yolov8-main/image_box"
...
while cap.isOpened():
    ...
    if success:
        ...
        if len(results[0].boxes) > 0:
            # Tạo tên tệp cho ảnh không có bounding box
            timestamp = int(time.time())
            filename_no_box = os.path.join(
                save_directory, f"captured_frame_no_box_{timestamp}.jpg")
            # Lưu ảnh không có bounding box
            cv2.imwrite(filename_no_box, frame)
            print(f"Đã lưu ảnh không có bounding box: {filename_no_box}")

            # Tạo tên tệp cho ảnh có bounding box
            filename_with_box = os.path.join(
                save_directory, f"captured_frame_with_box_{timestamp}.jpg")
            # Lưu ảnh có bounding box
            cv2.imwrite(filename_with_box, annotated_frame)
            print(f"Đã lưu ảnh có bounding box: {filename_with_box}")

            # Gửi tin nhắn với ảnh
            message = "Có người bị ngã ở đây nhé, đến đây!"
            asyncio.run(send_message_with_photo(
                TOKEN, CHAT_ID, message, filename_with_box))
            asyncio.run(send_message_with_photo(
                TOKEN, CHAT_ID, message, filename_no_box))
